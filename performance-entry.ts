...
function hasImageInput(body: Record<string, any>): boolean {
...
function hasPdfInput(body: Record<string, any>): boolean {
  const messages = Array.isArray(body.messages) ? body.messages : [];
  return messages.some((message: any) =>
...
function responseFromSnapshot(snapshot: CachedResponse): Response {
...
// First wrapper: optimize NVIDIA requests before the PDF bridge captures fetch.
// OCR keeps the existing multimodal model; the text-only final extraction uses the
// faster Nemotron 3.5 Lightning model. This changes backend inference only, not UI.
globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
  const url = typeof input === "string" ? input : input instanceof URL ? input.toString() : input.url;
  if (!url.startsWith(NVIDIA_URL) || typeof init?.body !== "string") {
  ...
  if (hasImageInput(rewritten)) {
    ...
    rewritten.chat_template_kwargs = {
      ...(isObject(rewritten.chat_template_kwargs) ? rewritten.chat_template_kwargs : {}),
...
// Second wrapper: deduplicate identical PDF analyses and cache successful results.
// This avoids paying the OCR + final-model latency again for repeated uploads and
// prevents duplicate concurrent scans of the same document.
